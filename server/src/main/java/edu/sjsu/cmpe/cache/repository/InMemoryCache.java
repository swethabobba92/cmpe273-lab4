...
public class InMemoryCache implements CacheInterface {
    ...
    @Override
    public String delete(Entry deleteEntry) {
       // checkNotNull(deleteEntry, "newEntry instance must not be null");
        inMemoryMap.remove(deleteEntry.getKey());

        return deleteEntry.getKey() + " removed from Cache";
    }
